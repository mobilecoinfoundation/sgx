...
pub use mc_sgx_tservice_types::{SealedData, UnsealedData};
...
pub trait Seal<T> {
    ...
    /// Seals the data
    fn seal_data(&self) -> Result<SealedData<T>>;
}

impl<T: AsRef<[u8]>> Seal<Vec<u8>> for UnsealedData<T> {
    /// Returns the size needed to seal the `data`
    ///
    /// # Arguments
    /// * `data` - The data to be sealed
    ...
    fn seal_data(&self) -> Result<SealedData<Vec<u8>>> {
        ...
        SealedData::try_from(sealed_data).map_err(|_| Error::Unexpected)
    }
}
...
mod test {
    ...
    #[test]
    fn sealed_data_size() {
        let data = UnsealedData::new(b"12345678".as_slice(), Some(b"123".as_slice()));
    ...
    fn sealed_data_size_no_mac() {
        let data = UnsealedData::new(b"1234567", None);
        let expected_size = mem::size_of::<sgx_sealed_data_t>() + data.len();
        assert_eq!(data.sealed_data_size(), Ok(expected_size));
    }
}
