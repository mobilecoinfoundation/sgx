...
pub trait Generate<T> {
    ...
    ///
    /// # Errors
    /// - [Error::SgxStatus(x)] when there is an error from the underlying SGX
    ///     api.
    fn generate(enclave: &Enclave) -> Result<T, Error>;
}

impl Generate<Quote> for Quote {
    fn generate(enclave: &Enclave) -> Result<Self, Error> {
        set_in_process_enclave_paths()?;
...
fn set_in_process_enclave_paths() -> Result<(), Error> {
    ...
    for (path, enclave) in [
        ...
        set_in_process_enclave_path(path, enclave)?
    }
    Ok(())
}

fn set_in_process_enclave_path(path_type: sgx_ql_path_type_t, enclave: &str) -> Result<(), Error> {
...
mod tests {
    ...
    fn generate_quote_for_enclave() {
        let enclave = EnclaveBuilder::new(ENCLAVE)
            ...
            .create()
            .unwrap();
        let quote = Quote::generate(&enclave).unwrap();
