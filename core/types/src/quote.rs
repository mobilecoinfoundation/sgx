...
use crate::{attestation_key::QuoteSignatureType, impl_newtype_for_bytestruct, new_type_accessors_impls, IsvSvn, ReportBody, FfiError};
use core::mem;
...
pub trait BaseQuote {
    ...
    /// The signature type
    fn signature_type(&self) -> Result<QuoteSignatureType, FfiError> {
...
mod test {
    ...
    fn quote_from_bytes_1x() {
        ...
        assert_eq!(quote.signature_type().unwrap(), QuoteSignatureType::UnLinkable);
    ...
    fn quote_from_bytes_2x() {
        ...
        assert_eq!(quote.signature_type().unwrap(), QuoteSignatureType::Linkable);
